...
static void scene_layer_surface_handle_layer_surface_destroy(
	...
	sway_scene_node_destroy(&scene_layer_surface->tree->node);
}
...
void sway_scene_layer_surface_v1_configure(
	...
	sway_scene_node_set_position(&scene_layer_surface->tree->node, box.x, box.y);
...
struct sway_scene_layer_surface_v1 *sway_scene_layer_surface_v1_create(
	...
	scene_layer_surface->tree = sway_scene_tree_create(parent);
	if (scene_layer_surface->tree == NULL) {
		free(scene_layer_surface);
		return NULL;
	}

	struct sway_scene_tree *surface_tree = sway_scene_subsurface_tree_create(
