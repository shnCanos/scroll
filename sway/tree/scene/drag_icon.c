...
static void drag_icon_handle_surface_commit(struct wl_listener *listener, void *data) {
	struct sway_scene_drag_icon *icon =
		wl_container_of(listener, icon, drag_icon_surface_commit);
	struct wlr_surface *surface = icon->drag_icon->surface;
	struct sway_scene_node *node = &icon->surface_tree->node;
	sway_scene_node_set_position(node,
...
struct sway_scene_tree *sway_scene_drag_icon_create(
	...
	icon->tree = sway_scene_tree_create(parent);
	if (!icon->tree) {
		free(icon);
		return NULL;
	}

	icon->surface_tree = sway_scene_subsurface_tree_create(
